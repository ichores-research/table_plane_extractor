...
def get_objects_on_table(req, target_frame):
    ...
    for plane_bb in response.plane_bounding_boxes.boxes:
        ...
        scene = transformPointCloud(scene_rel_to_camera, response_header.frame_id, camera_frame_id, tf_buffer)
        scene_o3d = orh.rospc_to_o3dpc(scene)

        # get bounding box above table
        bb_above_table = ros_bb_to_o3d_bb(plane_bb)
        bb_above_table.center = (bb_above_table.center[0], bb_above_table.center[1], bb_above_table.center[2]+max_obj_height/2.0+bb_above_table.extent[2])
        bb_above_table.extent = (bb_above_table.extent[0]+0.04, bb_above_table.extent[1]+0.04, bb_above_table.extent[2]+max_obj_height)
        ...
        scene_above_table_ros = transformPointCloud(scene_above_table_ros, target_frame, response_header.frame_id, tf_buffer)
...
def get_object_bbs(req):
    ...
    '''
    enable_rviz_visualization = rospy.get_param('/get_objects_on_table/enable_rviz_visualization')
    if enable_rviz_visualization:
        pub = rospy.Publisher('objectsOnTableVisualizer', MarkerArray, queue_size=10)
    ...
    ros_bb_arr = o3d_bb_list_to_ros_bb_arr(o3d_bb_arr, target_frame, rospy.get_rostime())
...
def get_objects_on_table_server():
    ...
    s_bb = rospy.Service('/objects_on_table/get_bounding_boxes', GetBBOfObjectsOnTable, get_object_bbs)
    s_pc = rospy.Service('/objects_on_table/get_point_clouds', GetPCOfObjectsOnTable, get_object_pcs)
